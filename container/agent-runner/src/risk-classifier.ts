...
const RED_BASH_PATTERNS = [
  ...
  { pattern: /\bgit\s+checkout\s+main\b/, reason: 'Switching to main branch — use feature branch only' },
  { pattern: /\bgit\s+merge\b/, reason: 'Merging branches — use /merge command instead' },
  ...
  { pattern: /\bcat\b.*(?:credentials?|\.pem|\.key\b|id_rsa|\.pfx|\.p12)\b/, reason: 'Reading credential file' },
  { pattern: /\becho\b.*\$[A-Z_]*(?:PASSWORD|SECRET|TOKEN|API_KEY|PRIVATE_KEY)[A-Z_]*\b/, reason: 'Printing secret environment variable' },
];
 ...
 */
export function classifyRisk(toolName: string, toolInput: unknown): RiskResult {
  ...
  if (toolName === 'Bash') {
    ...
    for (const { pattern, reason } of RED_BASH_PATTERNS) {
      if (pattern.test(command)) return { level: 'red', reason };
    }
    for (const { pattern, reason } of YELLOW_BASH_PATTERNS) {
      if (pattern.test(command)) return { level: 'yellow', reason };
    }
